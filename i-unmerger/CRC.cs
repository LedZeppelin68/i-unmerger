...
namespace i_unmerger
{
    class CRC
    {
        ...
        internal static void calculate_edc(byte[] temp_block, int mode)
        {
            UInt32 edc = 0;
            int count = 0;
            var i = 0;
            ...
            while (i != count)
            {
                edc = (UInt32)((edc >> 8) ^ CRC.edc_lut[(edc ^ (temp_block[offset + i++])) & 0xff]);
            }
            byte[] ar_edc = BitConverter.GetBytes(edc);

            ar_edc.CopyTo(temp_block, offset + count);

            //for (i = 0; i < 4; i++)
            //{
            //    temp_block[i + offset + count] = ar_edc[i];
            //}
        }

        internal static void calculate_eccp(byte[] temp_block)
        {
            ...
            for (major = 0; major < major_count; major++)
            {
                ...
                temp_block[2076 + major] = ecc_a;
                temp_block[2076 + major + major_count] = (byte)(ecc_a ^ ecc_b);
            }
        }

        internal static void calculate_eccq(byte[] temp_block)
        {
            UInt32 major_count, minor_count, major_mult, minor_inc;
            major_count = 52;
            minor_count = 43;
            ...
            for (major = 0; major < major_count; major++)
            {
                ...
                ecc_a = ecc_b_lut[ecc_f_lut[ecc_a] ^ ecc_b];
                temp_block[2076 + 172 + major] = ecc_a;
                temp_block[2076 + 172 + major + major_count] = (byte)(ecc_a ^ ecc_b);
            }
        }
    }
}
